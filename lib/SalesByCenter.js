...
export default class SalesByCenter extends React.Component
{
	static defaultProps = {
		...
		pending: false, 
		groupBy: [], 
		groupByChange: groupBy => {}, 
	};
	render()
	{
		const { pending } = this.props;
		const { fetchSalesByCenterParams } = this.props;
		const { kjob, syozok, tokuno } = fetchSalesByCenterParams;
		const { salesByCenter, groupBy } = this.props;
		const { groupByChange } = this.props;
		...
		return (
			<div className="sales-by-center">
				...
				<GroupBy groupBy={groupBy} onChange={groupByChange}/>
...
class Section extends React.Component
{
	...
	render()
	{
		...
		if (sortBy !== '')
		{
			...
			const name = match[1];
			const order = match[2];
			body = (body = [...body]).sort((a, b) =>
			{
				const option = order === 'asc' ? 1 : -1;
				const compare = a[name] > b[name] ? 1 : -1;
				...
				<table className="table is-narrow is-bordered is-striped">
					<thead>
						...
						{columns.map(({ name, title }, i) =>
							<th key={i} 
								className={classnames(name,'clickable')}
								onClick={e => this.onColumnClick(name)}
							>
								{title}

								<span className="icon sort-mark">
									<i className={classnames('fas', {
										'fa-sort-up'  : sortBy === `${name}|asc`, 
										'fa-sort-down': sortBy === `${name}|desc`, 
									})}
									></i>
								</span>
							</th>
