...
export const salesByCenterSaga = function *()
{
	yield takeLatest('FETCH_SALES_BY_CENTER', function *(action)
	{
		...
		yield put({ type: 'PENDING_SALES_BY_CENTER', payload: true });
		yield put({ type: 'SALES_BY_CENTER', payload: yield call(fetchSalesByCenter, params) });
		yield put({ type: 'PENDING_SALES_BY_CENTER', payload: false });
	});
}; 
...
const fetchSalesByCenter = async params =>
{
	try
	{
		return await axios('http://laravel.shokuryu-dwh.zeus.sss/sales-by-center', {
			params, 
		}).then(res => res.data);
	}
	catch (err)
